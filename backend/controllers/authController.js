  ...
  try {
    ...
    if (!userId) {
      return response(
        res,
        404,
        "unauthorized! pls login before loging this app "
      );
    }
    const user = await User.findById(userId);
    if (!user) {
...
export const getALLUser = async (req, res) => {
  try {
    ...
    const otherUsers = await User.find({ _id: { $ne: userId } })
      ...
      .lean(); // makes Mongoose return plain JavaScript objects instead of Mongoose documents.
    ...
    const usersWithConversation = await Promise.all(
      ...
      otherUsers.map(async (user) => {
        ...
        }; //If the property already exists in user, it will be overwritten.
        //If the property doesn’t exist, it will just be added.
      })
    );
